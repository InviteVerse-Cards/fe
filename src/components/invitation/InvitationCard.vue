...
const publicUrl = computed(() =>
  props.invitation.status === "published"
    ? `${import.meta.env.VITE_APP_URL}/i/${props.invitation.slug}`
    : null,
);
</script>
...
<template>
  <div
    class="flex flex-col rounded-xl border border-gray-200 bg-white shadow-sm hover:shadow-md transition-shadow"
  >
    <!-- Color strip -->
    <div
      ...
      :style="{
        backgroundColor:
          (invitation.theme_config as any)?.primary_color || '#6366F1',
      }"
    />
    ...
    <div class="flex-1 p-4">
      ...
      <div class="space-y-1 text-xs text-gray-500">
        ...
        <p v-if="invitation.published_at">
          📅 Xuất bản
          {{ new Date(invitation.published_at).toLocaleDateString("vi-VN") }}
        </p>
      </div>
